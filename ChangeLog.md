...
- A uniform's declared default reaches the interpreter, not just the compiled shader (onejs-react 0.1.52, onejs-unity 0.5.12)
- `<ShaderProgram>` checks uniform names against the program it was given (onejs-react 0.1.52)
...
- Edit-mode preview: your React UI renders in the Game view without ever entering Play mode

## New Features

- Hot reload in both edit and play mode, with `onPlay`/`onStop` lifecycle hooks and automatic cleanup of JS-created objects
- `JSPad`: a second MonoBehaviour with an inline code editor for instant prototyping, no project setup
- Built-in Tailwind-style JIT utility generator and CSS Modules, both embedded in the bundle (no external dependencies)
- ES6 imports of C# namespaces: `import { Vector3 } from "UnityEngine"`
- Zero-alloc C# interop: FastPath accessors, pre-registered bindings, batched style and vector-drawing commands
- 2D particle engine and GPU compute bridge rendering inside UI Toolkit elements
- Web APIs out of the box: `fetch`, `WebSocket`, `localStorage`, `URL`, base64, timers
- `UICartridge` content packs: drop a single asset onto a JSRunner to install themes and effects
- `onejs-ui`: a themeable React component library with premade themes (Pixel, Kawaii, Sketch)
- AI-native development via the Unity MCP Server companion package

## Workflow Improvements

- Scene Folder workflow: each app lives next to its scene in a git-friendly folder, tilde-suffixed so Unity ignores sources
- The editor manages the esbuild watcher automatically: save a file, the UI hot-reloads
- `unity-types` npm package ships generated TypeScript definitions for the whole `CS.*` surface
- The full JS side verifies from the CLI (`npm run build` / `typecheck`), no editor needed

## Other Enhancements

- All core types now live under the `OneJS` namespace (with backward-compatible resolution for older bundles)
- Built-in stability monitoring: handle-table and async-task-queue leak detection with automatic warnings
- Player builds embed the JS bundle automatically, nothing passes through StreamingAssets
