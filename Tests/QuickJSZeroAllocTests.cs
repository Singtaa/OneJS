...
public class QuickJSZeroAllocPerformanceTests {
    ...
    /// <summary>
    /// Tests that InvokeCallbackNoAlloc has low overhead.
    /// This is the core mechanism for zero-alloc per-frame tick.
    ///
    /// Note: This test requires QuickJSUIBridge for native callback infrastructure.
    /// The native qjs_invoke_callback only works when the bridge's callback table
    /// is properly initialized, which doesn't happen with basic QuickJSContext.
    /// </summary>
    [UnityTest]
    [Ignore("Requires QuickJSUIBridge - native callback infrastructure not available in basic QuickJSContext")]
    public IEnumerator InvokeCallbackNoAlloc_LowOverhead() {
        // Define a minimal __registerCallback for testing
        // (In production, this comes from QuickJSBootstrap.js)
        _ctx.Eval(@"
            var __callbackId = 0;
            var __callbacks = {};
            ...
            globalThis.__invokeCallback = function(id, arg) {
                var fn = __callbacks[id];
                if (fn) fn(arg);
            };
        ");

        // Register a callback
        int handle = int.Parse(_ctx.Eval(@"
            var counter = 0;
            __registerCallback(function(timestamp) {
                counter++;
            });
        "));

        Assert.Greater(handle, 0, "Callback should be registered");

        // Warm up
        for (int i = 0; i < 100; i++) {
            _ctx.InvokeCallbackNoAlloc(handle, i);
        }
        ...
        Debug.Log($"[ZeroAlloc] InvokeCallbackNoAlloc: {sw.ElapsedMilliseconds}ms for {iterations} calls");
        Debug.Log($"[ZeroAlloc] Memory: {bytesAllocated} bytes ({bytesAllocated / (float)iterations:F2} per call)");

        // Should be very low allocation
        Assert.Less(bytesAllocated, 10000, "InvokeCallbackNoAlloc should have minimal allocation");
