...
namespace OneJS.Tests.Editor {
    ...
    public class CartridgeUtilsTests {
        ...
        public void Generate_WithVersion_WritesVersionLineInHeader() {
            var cartridge = CreateTestCartridge("versionedCart");
            SetCartridgeVersion(cartridge, "1.2.0");

            var dts = CartridgeTypeGenerator.Generate(cartridge);

            StringAssert.Contains($"{CartridgeTypeGenerator.VersionLinePrefix}1.2.0", dts);
            Object.DestroyImmediate(cartridge);
        }

        [Test]
        public void Generate_WithoutVersion_OmitsVersionLine() {
            var cartridge = CreateTestCartridge("unversionedCart");

            var dts = CartridgeTypeGenerator.Generate(cartridge);

            StringAssert.DoesNotContain(CartridgeTypeGenerator.VersionLinePrefix, dts);
            Object.DestroyImmediate(cartridge);
        }

        [Test]
        public void ParseVersion_RoundTripsThroughGenerate() {
            var cartridge = CreateTestCartridge("roundTripCart");
            SetCartridgeVersion(cartridge, "2.0.1");

            var dts = CartridgeTypeGenerator.Generate(cartridge);

            Assert.AreEqual("2.0.1", CartridgeTypeGenerator.ParseVersion(dts));
            Object.DestroyImmediate(cartridge);
        }
        ...
        public void ParseVersion_OnlyScansTheCommentHeader() {
            ...
            var text = "// Auto-generated by OneJS Cartridge: x\n\n// Version: 9.9.9\nexport {};";
            Assert.IsNull(CartridgeTypeGenerator.ParseVersion(text));
            Assert.IsNull(CartridgeTypeGenerator.ParseVersion(null));
            Assert.IsNull(CartridgeTypeGenerator.ParseVersion(""));
        }

        [Test]
        public void GetExtractedVersion_AfterExtract_MatchesAssetVersion() {
            var cartridge = CreateTestCartridge("extractVersion");
            SetCartridgeVersion(cartridge, "1.0.0");

            CartridgeUtils.ExtractCartridges(_testBasePath, new List<UICartridge> { cartridge }, false);

            Assert.AreEqual("1.0.0", CartridgeUtils.GetExtractedVersion(_testBasePath, cartridge));
            Object.DestroyImmediate(cartridge);
        }

        [Test]
        public void GetExtractedVersion_UnversionedExtraction_ReturnsNull() {
            var cartridge = CreateTestCartridge("noVersion");

            CartridgeUtils.ExtractCartridges(_testBasePath, new List<UICartridge> { cartridge }, false);

            Assert.IsNull(CartridgeUtils.GetExtractedVersion(_testBasePath, cartridge));
            Object.DestroyImmediate(cartridge);
        }

        [Test]
        public void ExtractCartridges_SkipExisting_WarnsWhenVersionDiffers() {
            var cartridge = CreateTestCartridge("staleCart");
            SetCartridgeVersion(cartridge, "1.0.0");
            var cartridges = new List<UICartridge> { cartridge };

            CartridgeUtils.ExtractCartridges(_testBasePath, cartridges, false);
            SetCartridgeVersion(cartridge, "1.1.0");
        ...
        [Test]
        public void ExtractCartridges_SkipExisting_NoWarningWhenVersionMatches() {
            var cartridge = CreateTestCartridge("freshCart");
            SetCartridgeVersion(cartridge, "1.0.0");
            var cartridges = new List<UICartridge> { cartridge };
