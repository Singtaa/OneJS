...
namespace OneJS {
    public static partial class QuickJSNative {
        ...
        static Type ResolveType(string fullName) {
            ...
            // Core types moved from the global namespace to OneJS in 3.1.0;
            // bundles built against older typings still request the bare name.
            if (type == null && !fullName.Contains(".")) {
                type = ResolveType("OneJS." + fullName);
            }
