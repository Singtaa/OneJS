using System;
using System.Collections;
using System.Collections.Generic;
using Jint;
using Jint.Native;
using UnityEngine;

namespace OneJS.Engine.JSGlobals {
    public class SetInterval {
        static int ID = 0;
        static Dictionary<int, IEnumerator> ID_DICT = new Dictionary<int, IEnumerator>();

        public static void Reset() {
            foreach (var kvp in ID_DICT) {
                CoroutineUtil.Stop(kvp.Value);
            }
            ID_DICT.Clear();
        }

        public static void Setup(ScriptEngine engine) {
            engine.JintEngine.SetValue("setInterval", new Func<JsValue, float, int>((handler, timeout) => {
                ID++;
                Action action = () => { handler.As<Jint.Native.Function.FunctionInstance>().Call(); };
                var routine = DoForever(action, timeout / 1000f);
                ID_DICT.Add(ID, routine);
                CoroutineUtil.Start(routine);
                return ID;
            }));
            engine.JintEngine.SetValue("clearInterval", new Action<int>((id) => {
                if (ID_DICT.ContainsKey(id)) {
                    CoroutineUtil.Stop(ID_DICT[id]);
                    ID_DICT.Remove(id);
                }
            }));
        }
        
        public static IEnumerator DoForever(Action action, float t) {
            while (true) {
                yield return new WaitForSeconds(t);
                action();
            }
        }
    }
}