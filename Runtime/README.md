...
the VM. The VM draws until the browser has compiled it, and for good after a
compile error, which the host logs once. The host reads three handles private to
...
`SetCompiled(false)` keeps an element on the VM, which is how the two are
compared (`Tools/sl-web-parity` in the container).
