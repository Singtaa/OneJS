...
- `StreamingAssets/onejs/assets/` is generated: the build processor rebuilds it from every app's `~/assets/` on each build, so anything put there by hand is removed. What is normally in there is the last build's output, not your files. Keep hand placed files elsewhere under `StreamingAssets/`. Two apps cannot ship the same relative path, and the comparison is case insensitive. Two apps a build scene reached fail the build naming both; if a prefab reached either of them it warns instead, because a prefab does not say whether it ships.
- **Every JSRunner in a scene or a prefab is built**, unless **Include In Build** is off on it. Prefabs are walked because an app can ship through Addressables, a `Resources/` folder, an AssetBundle or a plain prefab reference, none of which any scene walk sees; before this, such an app shipped with no bundle and the player ran OneJS's default placeholder app. Turn Include In Build off for a runner kept in the project but never shipped, which also keeps its files out of the collision check.
- Prefab apps are baked from `BuildPlayerProcessor.PrepareForBuild`, not from `OnPreprocessBuild` where scene apps are. Every `BuildPlayerProcessor` runs before every `IPreprocessBuildWithReport`, and Addressables builds its content from one, so a bundle baked in `OnPreprocessBuild` is already too late for the Addressable content carrying that prefab. Measured, not assumed: with the walk in the old hook the prefab still ends up holding a bundle reference, so the project looks right while the shipped bundle has no app in it, and only the *next* build picks it up.
